...
public class UtGen {
	...
	/*
	 * mp = "memory pointer" coloca al registro[6] a apuntar al tope de la memoria
	 * (para almacenamiento de variable temporales como t1, t2, etc.)
	 */
	public static int MP = 6;

	/*
	 * gp = "global pointer" coloca al registro[5] a apuntar al inicio de la memoria
	 * para el almacenamiento de variables globales
	 */
	...
	/*
	 * Este procedimiento emite sentencias RO (Solo Registro) de la TM opcode r,s,t
	 * r[r] = reg[s] op reg[t]
	 * 
	 * op = codigo de la operacion r = registro destino s = primero registro
	 * operando t = segundo registro operando c = comentario a emitir en modo debug
	 */
	...
	/*
	 * Este procedimiento emite sentencias RM (Registro a memoria) de la TM opcode
	 * r,d(s) a=d+reg[s] reg[PC_REG]=a o reg[r] = x (dependiendo de instruccion de
	 * carga)
	 * 
	 * op = codigo de la operacion r = registro destino d = desplazamiento s =
	 * registro con la direccion base c = comentario a emitir en modo debug
	 */
	...
	/*
	 * La funcion emitirSalto, salta "cantidad" de localidades de codigo para el
	 * reajuste posterior. Tambien devuelve la posicion actual del codgio generado
	 */
	 ...
	 * actual a la posicion mas alta alcanzada previamente y que no ha sido emitida
	 * aun
	 */
	...
	/*
	 * El procedimiento emitirRM_Abs convierte una referencia absoluta a una
	 * referencia relativa al pc (program counter, registro [7]) cuando se emite una
	 * instruccion de tipos registro a memoria (RM) de la TM
	 *
	 * op = codigo de la operacion r = registro destino a = la localidad absoluta en
	 * memoria c = comentario a emitir en modo debug
	 */
	public static void emitirRM_Abs(String op, int r, int a, String c) {
		System.out.print((instruccionActual) + ":       " + op + "       " + r + "," + (a - (instruccionActual + 1))
				+ "(" + PC + ")");
		++instruccionActual;
		if (debug)
			System.out.print("      " + c);
